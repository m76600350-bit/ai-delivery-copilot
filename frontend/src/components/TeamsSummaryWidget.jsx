...
export default function TeamsSummaryWidget({ dashboardFilters, lastSyncedAt, localPeriod, onLocalPeriodChange, onNavigateToTasks, onRemove, onExpand, fullScreen }) {
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border border-gray-200 p-5 relative flex flex-col ${widgetHeightClass(fullScreen)}`}>
      ...
      <p className="text-xs text-gray-400 mb-3 shrink-0">таблица · {localFilterActive ? `свой фильтр: период ${PERIOD_OPTIONS.find((o) => o.value === localPeriod)?.label.toLowerCase()}` : 'период дашборда'}</p>
