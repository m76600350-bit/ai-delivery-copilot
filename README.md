...
9. **Спринты (Jira Agile API, только бэкенд)** — `POST /api/jira/sync` дополнительно тянет данные о спринтах через `/rest/agile/1.0/*`, отдельным best-effort шагом после основной синхронизации issues (`syncSprintsForCloud` в [routes/jira.js](backend/routes/jira.js)):
   ...
   - UI для «Команды»/«Спринты» на этих данных — отдельная задача; пока это только данные в БД.

   ⚠️ Не проверено на реальном Jira-сайте — если у текущего OAuth-приложения не хватает scope для Agile API (например, оно настроено на granular-скоупы вместо classic и Agile-эндпоинты требуют отдельных `read:board-scope:jira-software`/`read:sprint:jira-software`, которых нет в списке `scope` в [routes/auth.js](backend/routes/auth.js) — там сейчас `read:jira-work read:jira-user offline_access`), Agile-запросы будут отвечать `403`. В таком случае `sprints`/`issue_sprints` останутся пустыми при в остальном успешном синке issues, но это не пройдёт незамеченным: первая же ошибка по любому проекту/доске/спринту (`firstError` в `syncSprintsForCloud`) попадает в `sprintSyncError` в ответе `/sync` и в `error` соответствующей строки `sync_history` — так что `403`/scope-проблема будет видна в истории синхронизаций в «Настройках», даже если сам синк issues прошёл успешно.
