...
7a. **Раздел «Настройки»** ([Settings.jsx](frontend/src/components/Settings.jsx)) — три блока:
   - **Подключение Jira** — только статус/домен (`site_url`) и «Переподключить Jira» (снова ведёт на `/api/auth/login`); полей для ручного ввода URL/токена нет и не будет, т.к. вся авторизация идёт через OAuth. Там же — «Настройки полей» (открывает существующий `FieldMapping.jsx`, не путать с фильтром синхронизации ниже).
   - **Фильтр синхронизации** ([SyncFilter.jsx](frontend/src/components/SyncFilter.jsx)) — dropdown «Проект» (список из живого `GET /rest/api/3/project/search`), «Тип задачи» и «Команда» (значения, реально встречающиеся в уже синхронизированных issues — `DISTINCT issue_type`/`team` по БД), и число «Период (дней)». Каждое изменение сохраняется сразу через `POST /api/jira/sync-filter` (число — с debounce 600 мс, чтобы не слать запрос на каждый символ), без отдельной кнопки «Сохранить». Пользователь никогда не видит и не пишет JQL — `buildJqlFromFilter` в [routes/jira.js](backend/routes/jira.js) собирает его из сохранённого фильтра при каждом sync (`project in (...)`, `issuetype in (...)`, `updated >= -Nd`, через `jqlQuote` с экранированием кавычек/бэкслэшей). Фильтр по команде в JQL не выражается надёжно (это либо кастомное поле неизвестного на момент написания типа, либо просто labels-фолбэк) — применяется постфактум в JS после `mapJiraFields`, до похода в changelog и записи в БД, так что отфильтрованные issues не тратят лишний запрос к Jira.
   - **Синхронизация** ([SyncHistory.jsx](frontend/src/components/SyncHistory.jsx)) — таблица последних 20 запусков из новой таблицы `sync_history` (Время/Источник/Задач/Итог, с текстом ошибки в `title` при наведении на «ошибка») и кнопка «Обновить сейчас» (тот же `syncJira()`, с тем же live-прогрессом через `useSyncProgress`, что и кнопки на дашборде).
