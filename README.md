...
11. **Раздел «Спринты»** ([Sprints.jsx](frontend/src/components/Sprints.jsx), backend в [routes/sprints.js](backend/routes/sprints.js), смонтирован как `/api/sprints`) — burndown, cumulative flow, история спринтов и риски активного спринта, целиком из уже засинканных данных.
    ...
    - **Burndown** — по каждому дню спринта (от `start_date` до `min(today, end_date)` для активного, иначе весь диапазон) сумма SP issues, чья категория на конец этого дня (через `issue_status_events`) ещё не `done`. Идеальная линия — прямая от текущего общего SP спринта (то же число, что в KPI «Взято») до 0 на `end_date`. «Отставание N SP» = `факт − идеал` на последний день, показывается только когда положительное (т.е. задач по факту остаётся больше, чем должно быть по графику) и только для активного спринта.
