...
│       ├── jira.js       # sync/status/issues/fields/field-mapping/tasks/wip-limits
...
10. **Раздел «Команды»** ([Teams.jsx](frontend/src/components/Teams.jsx), backend в [routes/teams.js](backend/routes/teams.js), смонтирован как `/api/teams`) — целиком читает уже засинканные данные, живых запросов к Jira не делает.
    ...
    - **WIP-лимиты** — блок в «Настройках» ([WipLimits.jsx](frontend/src/components/WipLimits.jsx)): по числовому полю на каждый статус с категорией `indeterminate`, реально встречающийся в засинканных данных (не хардкод), сохраняется сразу при изменении (debounce 600 мс) через `POST /api/jira/wip-limits` в таблицу `wip_limits`. Глобально по сайту, не per-team — у команды в таблице «Команды» это сумма всех настроенных лимитов.
    - **Таблица «Команды»** — на команду: людей (уникальных assignee), «Загрузка» (текущий WIP в indeterminate-статусах / сумма лимитов — если лимиты не настроены вообще, показывается только число), Velocity SP (сумма story points всех задач выбранного спринта — или, если фильтр «Спринт» не выбран, последнего спринта команды: активного, иначе последнего закрытого), Cycle time (среднее по завершённым задачам команды), Баги % (доля `issue_type = Bug`), Блокеры (см. выше), Здоровье.
    - **Здоровье** — три сигнала, каждый сравнивается с историей самой команды (`resolveLastSprint`/health-блок в `routes/teams.js`): (1) текущий WIP / лимит, срабатывает при `> 1.0`; (2) средний cycle time завершённых задач за последние 30 дней / средний cycle time по всем завершённым задачам команды, срабатывает при `> 1.3`; (3) SP завершённых задач последнего закрытого спринта / средняя по предыдущим (до 5) закрытым спринтам, срабатывает при `< 0.7`. Итог — «норма» (0 сработавших), «риск» (1), «перегруз» (2+). Если у команды меньше 3 завершённых задач за всё время или нет ни одного закрытого спринта — «недостаточно данных», сигналы вообще не считаются.
